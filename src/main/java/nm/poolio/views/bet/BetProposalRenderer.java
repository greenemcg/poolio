...
public class BetProposalRenderer
    ...
    private Component[] createAmountDialog(GameBet gameBet) {
        ...
        IntegerField integerField = new IntegerField("Bet Amount");
        ...
        return new Component[]{new H4("Choose Amount To Wager"), integerField};
    }
    ...
    private Button createAcceptButton(
        ...
        if (playerAlreadyMadeBet(gameBet)) {
            ...
            Button b = new Button("Your Game Wagers");
            b.setEnabled(false);
            return b;
        } else {
            var funds = poolioTransactionService.getFunds(player);
            if (Boolean.TRUE.equals(gameBet.getBetCanBeSplit())) {
                ...
                Button b =
                        new Button(
                                "Accept this Wager for $%d to $%d betting %s (%s)"
                ...
                if (gameBet.getAmount() > funds) {
                    ...
                    b =
                            new Button(
                                    "Accept this Wager for $%d betting %s (%s)"
    ...
    public HorizontalLayout render(GameBet gameBet) {
        ...
        if (gameBet.getProposer().equals(player)) {
            ...
            if (!gameBet.getAcceptorTransactions().isEmpty()) editProposal.setEnabled(false);
    ...
    private Button createEditButton(GameBet gameBet) {
        Button editProposal = new Button("Edit Wager");
        editProposal.setIcon(EDIT_ICON.create());
                    ...
                    dialog.addConfirmListener(
                            event -> {
                                ...
                                List<PoolioTransaction> refunds =
                                        refund(gameBet, poolioTransactionService, PoolioTransactionType.CANCEL_WAGER);
    ...
    private void openAcceptBetConfirmDialog(
        ...
        if (gameBet.getProposer().equals(player)) {
            ...
            ConfirmDialog dialog = new ConfirmDialog();
            dialog.setHeader("Accept Wager : Are you sure");
            NflTeam otherTeamNotPicked = getNflTeamNotPicked(gameBet, nflGame);
            String spreadString = createSpreadString(gameBet, otherTeamNotPicked, nflGame);
            dialog.setText(
                    "Accept this Wager for $%d betting %s (%s)"
            ...
            dialog.setCancelable(true);
            dialog.setConfirmText("Accept Wager from " + gameBet.getProposer().getName());
