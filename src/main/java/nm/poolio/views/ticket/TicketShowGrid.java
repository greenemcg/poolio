...
public interface TicketShowGrid extends PoolioGrid<NflGame> {
    ...
    private Component createStylesTeamComponent(
        ...
        if (pickedTeam != null) {
            ...
            String mark = (pickedTeam == teamToCheck) ? "✔ " : "✘ ";
    ...
    private void createWinnerText(Boolean pickCorrect, Style style) {
        if (pickCorrect == null) {
            style.set("font-style", "oblique");
        } else {
            style.set("color", pickCorrect ? "green" : "red");
            style.set(pickCorrect ? "font-weight" : "text-decoration", pickCorrect ? "bold" : "line-through");
        }
    }
