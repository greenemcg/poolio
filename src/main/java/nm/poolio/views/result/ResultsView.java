...
public class ResultsView extends VerticalLayout
  ...
  private void decorateGrid() {
    ...
    FooterRow footer = resultsGrid.appendFooterRow();

    games.forEach(
        g -> {
          if (g.getScore().isPresent()) {
            Span footerText = new Span(g.getAwayScore() + " - " + g.getHomeScore());
          footerText.getElement().getStyle().set("color", "var(--lumo-contrast-70pct)");
              footerText.getElement().getStyle().set("font-size", "var(--lumo-font-size-m)");
             // footerText.addClassNames(LumoUtility.TextColor.ERROR);
              footer.getCell(this.resultsGrid.getColumnByKey(g.getId())).setComponent(footerText);
          }
        });
  }
